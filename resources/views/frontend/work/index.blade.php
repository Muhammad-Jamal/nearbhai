                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="py-6 sorting" tabindex="0" aria-controls="invoice-list" rowspan="1"
                                        ...
                                        style="width: 67.875px;">Budget</th>
                                ...
                                @forelse ($works as $work)
                                    <tr role="row" class="odd">
                                        <td class="align-middle"><a href="dashboard-preview-invoice.html"><span
                                                    class="inv-number">{{ $work->id }}</span></a>
                                        </td>
                                        <td class="align-middle"><span class="inv-amount">{{ $work->name }}</span>
                                        </td>
                                        <td class="align-middle"><span
                                                class="text-primary pr-1"></span>{{ $work->email }}</td>
                                        <td class="align-middle"><span
                                                class="text-success pr-1"></span>{{ $work->phone }}</td>
                                        <td class="align-middle"><span
                                                class="text-success pr-1"></span>{{ $work->budget }}</td>
                                        <td class="action-buttons align-middle">
                                            <a href="{{ route('pages.work.detail',$work->id) }}"
                                            ...
                                            <a href="{{ route('work.edit',$work->id) }}"
                                                ...
                                                <a class="ebutton btn btn-danger" onclick="return confirm('Are you sure to delete this card ?')" href="{{route('work.delete', $work->id)}}"><i class="fa fa-trash"></i>Delete</a>
                                        </td>
                                    </tr>
                                @empty
